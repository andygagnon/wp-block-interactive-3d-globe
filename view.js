...
// Data for more detailed land mass approximations
const LAND_MASSES = {
    europe: [
        { lat: 60, lon: -10 }, { lat: 62, lon: 0 }, { lat: 65, lon: 10 },
        { lat: 68, lon: 20 }, { lat: 65, lon: 30 }, { lat: 55, lon: 30 },
        { lat: 50, lon: 25 }, { lat: 45, lon: 20 }, { lat: 40, lon: 15 },
        { lat: 38, lon: 10 }, { lat: 36, lon: 5 }, { lat: 35, lon: -5 },
        { lat: 40, lon: -8 }, { lat: 45, lon: -10 }, { lat: 50, lon: -8 },
        { lat: 55, lon: -5 }, { lat: 60, lon: -10 }
    ],
    northAmerica: [
        { lat: 60, lon: -160 }, { lat: 70, lon: -140 }, { lat: 75, lon: -120 },
        { lat: 65, lon: -100 }, { lat: 60, lon: -80 }, { lat: 50, lon: -70 },
        { lat: 40, lon: -75 }, { lat: 30, lon: -80 }, { lat: 20, lon: -85 },
        { lat: 10, lon: -100 }, { lat: 15, lon: -110 }, { lat: 25, lon: -120 },
        { lat: 35, lon: -125 }, { lat: 45, lon: -125 }, { lat: 55, lon: -140 },
        { lat: 60, lon: -150 }
    ],
    southAmerica: [
        { lat: 12, lon: -78 }, { lat: 5, lon: -75 }, { lat: 0, lon: -50 },
        { lat: -10, lon: -40 }, { lat: -25, lon: -35 }, { lat: -40, lon: -50 },
        { lat: -55, lon: -70 }, { lat: -45, lon: -80 }, { lat: -20, lon: -75 },
        { lat: 0, lon: -80 }, { lat: 5, lon: -85 }
    ],
    asia: [
        { lat: 50, lon: 40 }, { lat: 60, lon: 50 }, { lat: 70, lon: 80 },
        { lat: 75, lon: 120 }, { lat: 65, lon: 160 }, { lat: 50, lon: 170 },
        { lat: 40, lon: 180 }, { lat: 30, lon: 160 }, { lat: 20, lon: 140 },
        { lat: 10, lon: 120 }, { lat: 5, lon: 100 }, { lat: 10, lon: 80 },
        { lat: 20, lon: 60 }, { lat: 30, lon: 50 }
    ],
    africa: [
        { lat: 30, lon: 10 }, { lat: 35, lon: 20 }, { lat: 25, lon: 35 },
        { lat: 10, lon: 45 }, { lat: -10, lon: 50 }, { lat: -25, lon: 45 },
        { lat: -35, lon: 30 }, { lat: -30, lon: 20 }, { lat: -10, lon: 10 },
        { lat: 5, lon: -10 }, { lat: 20, lon: -15 }, { lat: 25, lon: 0 }
    ],
    australia: [
        { lat: -10, lon: 140 }, { lat: -15, lon: 145 }, { lat: -25, lon: 150 },
        { lat: -35, lon: 145 }, { lat: -40, lon: 140 }, { lat: -35, lon: 130 },
        { lat: -25, lon: 115 }, { lat: -15, lon: 125 }
    ],
    antarctica: [
        { lat: -70, lon: 0 }, { lat: -80, lon: 40 }, { lat: -85, lon: 80 },
        { lat: -88, lon: 120 }, { lat: -85, lon: 160 }, { lat: -75, lon: 180 },
        { lat: -70, lon: -150 }, { lat: -80, lon: -120 }, { lat: -85, lon: -90 },
        { lat: -80, lon: -40 }, { lat: -75, lon: 0 }
    ]
};
...
const initGlobe = (containerId) => {
    ...
    // Updated to a brighter green for better visibility
    const landMaterial = new THREE.MeshBasicMaterial({ color: 0x7CFC00 });

    // Iterate through each land mass and create its shape
    for (const continent in LAND_MASSES) {
        if (Object.hasOwnProperty.call(LAND_MASSES, continent)) {
            const points2D = LAND_MASSES[continent].map(p => new THREE.Vector2(p.lon, p.lat));
            const shape = new THREE.Shape(points2D);
            const geometry = new THREE.ShapeGeometry(shape);

            // To place the 2D shape on the 3D globe, we must manually
            // convert the vertices.
            const vertices = geometry.getAttribute('position');
            const newVertices = [];
            for (let i = 0; i < vertices.count; i++) {
                const lon = vertices.getX(i);
                const lat = vertices.getY(i);
                const pos = getCartesianCoordinates(lat, lon, 1.001); // Slightly larger radius to avoid z-fighting
                newVertices.push(pos.x, pos.y, pos.z);
            }
            const landGeometry = new THREE.BufferGeometry();
            landGeometry.setAttribute('position', new THREE.Float32BufferAttribute(newVertices, 3));
            landGeometry.index = geometry.index;
